      ...
      <div className="container mx-auto p-4 md:p-8 space-y-8">
            ...
            <>
              <div className="bg-white rounded-4xl p-6 shadow-sm border border-neutral-100">
                ...
                <p className="text-2xl font-black text-neutral-900 mt-1">
                  {formatCurrency(
                    goals.reduce((acc, g) => acc + Number(g.amountCurrent), 0),
                  )}
                </p>
              </div>
              <div className="bg-white rounded-4xl p-6 shadow-sm border border-neutral-100">
                ...
                <p className="text-2xl font-black text-blue-600 mt-1">
                  {formatCurrency(
                    goals.reduce((acc, g) => acc + Number(g.amountGoal), 0),
                  )}
                </p>
              </div>
              <div className="bg-neutral-900 rounded-4xl p-6 shadow-xl text-white">
                ...
                <p className="text-2xl font-black mt-1">
                  {goals.length > 0
                    ? Math.round(
                        (goals.reduce(
                          (acc, g) => acc + Number(g.amountCurrent),
                          0,
                        ) /
                          goals.reduce(
                            (acc, g) => acc + Number(g.amountGoal),
                            0,
                          )) *
                          100,
                      )
        ...
        <main>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!isLoading &&
              goals.map((goal) => {
                ...
                return (
                  <div
                    key={goal.id}
                    className="bg-white rounded-4xl p-6 shadow-sm border border-neutral-100 flex flex-col justify-between hover:shadow-md transition-all group"
                  >
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        <div
                          className={`p-3 rounded-2xl ${isFinished ? "bg-green-100 text-green-600" : "bg-neutral-100 text-neutral-900"}`}
                        >
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className="p-2 hover:bg-neutral-100 rounded-lg transition text-neutral-400"
                          >
                      ...
                      <h3 className="text-lg font-black text-neutral-900 leading-tight mb-1">
                        {goal.title}
                      </h3>
                        ...
                        <div className="flex justify-between items-end">
                          <div className="flex flex-col">
                            ...
                            <span className="text-sm font-black text-neutral-900">
                              {formatCurrency(goal.amountCurrent)}
                            </span>
                          </div>
                          <div className="flex flex-col text-right">
                            ...
                            <span className="text-sm font-bold text-neutral-400">
                              {formatCurrency(goal.amountGoal)}
                            </span>
                          </div>
                        </div>
            ...
            {!isLoading && (
              <button
                className="border-2 border-dashed border-neutral-200 rounded-4xl p-6 flex flex-col items-center justify-center text-neutral-400 hover:border-neutral-900 hover:text-neutral-900 transition-all group gap-3 min-h-80"
                onClick={handleCreate}
              >
