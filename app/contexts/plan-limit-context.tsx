...
import {
  PlanLimitModal,
  PlanLimitType,
} from "@/app/components/ui/plan-limit-modal";
import { PricingModal } from "@/app/components/ui/pricing-modal";
...
export function PlanLimitProvider({ children }: { children: React.ReactNode }) {
  ...
  const [planLimitType, setPlanLimitType] =
    useState<PlanLimitType>("transactions");
  const [pricingOpen, setPricingOpen] = useState(false);

  const openPlanLimitModal = useCallback(
    (limitType: PlanLimitType = "transactions") => {
