...
export default function TransactionsPage() {
  ...
  const handleDeleteTransaction = async (id: number, scope: "one" | "all" = "one") => {
    if (scope === "all" && !confirm("Excluir série?")) return;
    const res = await fetch(`http://localhost:3333/v1/transactions/${id}?scope=${scope}`, {
      method: "DELETE",
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      res = res.filter(t => t.title.toLowerCase().includes(term) || t.category.toLowerCase().includes(term));
    }
  ...
  return (
    <div className="min-h-screen w-full pb-20">
      ...
      <header className="bg-white border-b border-neutral-100 sticky top-0 z-10">
        <div className="container mx-auto px-4 md:px-8 py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-black text-neutral-900 tracking-tight">Histórico de Transações</h1>
              <p className="text-sm text-neutral-400 font-medium">Gerencie e filtre todos os seus lançamentos</p>
            </div>
      ...
      <main className="container mx-auto px-4 md:px-8 py-8">
        ...
        <div className="bg-white p-4 rounded-3xl border border-neutral-100 shadow-sm mb-6 flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center bg-neutral-50 rounded-2xl border border-neutral-100 p-1">
              <button onClick={prevMonth} className="p-2 hover:bg-white rounded-xl transition"><ArrowLeftIcon size={16} weight="bold" /></button>
              <span className="text-xs font-black px-3 capitalize min-w-[100px] text-center">{monthName}</span>
              <button onClick={nextMonth} className="p-2 hover:bg-white rounded-xl transition"><ArrowRightIcon size={16} weight="bold" /></button>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-neutral-50 flex justify-between items-center bg-neutral-50/30">
            <h3 className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">Lançamentos</h3>
      ...
      <TransactionDrawer 
        open={openDrawer} 
        onClose={() => { setOpenDrawer(false); setTransactionToEdit(null); }} 
